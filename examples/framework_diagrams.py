...
def create_dras5_state_machine(output_dir='outputs/figures'):
    ...
    ax.text(
        8,
        11.0,
        '5-State Risk Machine for Emergency Triage Decision Support',
        ha='center',
        va='top',
        fontsize=13,
        style='italic',
    ...
    for i, row in enumerate(table_data):
        ...
        for j, cell in enumerate(row):
            ...
            if i == 0:
                cell_box = Rectangle(
                    (x_offset, table_y - (i + 1) * cell_height),
                    cell_width[j],
                    cell_height,
                    facecolor='#333333',
                    edgecolor='black',
                ...
                if j == 0:
                    state_id = cell
                    cell_color = (
                        states[state_id]['color'] if state_id in states else '#F0F0F0'
                    )
                    alpha = 0.3
                else:
                    cell_color = '#FFFFFF'
            ...
            ax.text(
                x_offset + cell_width[j] / 2,
                table_y - (i + 0.5) * cell_height,
                cell,
                ha='center',
                va='center',
                fontsize=7,
    ...
    legend_elements = [
        mpatches.Patch(
            facecolor=COLORS['S1'],
            edgecolor='black',
            label='S1: Critical (R1)',
            alpha=0.6,
        ),
        mpatches.Patch(
            facecolor=COLORS['S2'],
            edgecolor='black',
            label='S2: High Risk (R2)',
            alpha=0.6,
        ),
        mpatches.Patch(
            facecolor=COLORS['S3'],
            edgecolor='black',
            label='S3: Moderate Risk (R3)',
            alpha=0.6,
        ),
        mpatches.Patch(
            facecolor=COLORS['S4'],
            edgecolor='black',
            label='S4: Low Risk (R4)',
            alpha=0.6,
        ),
        mpatches.Patch(
            facecolor=COLORS['S5'],
            edgecolor='black',
            label='S5: Minimal Risk (R5)',
            alpha=0.6,
        ),
    ]
    ax.legend(
    ...
    # Save figure
    plt.savefig(
        f'{output_dir}/fig8_dras5_state_machine.png', dpi=600, bbox_inches='tight'
    )
...
def create_orasr_routing_diagram(output_dir='outputs/figures'):
    ...
    ax.text(
        8,
        9.0,
        'Safe Care Pathway Routing with Audit Trails and Safety Nets',
        ha='center',
        va='top',
        fontsize=13,
        style='italic',
    ...
    ax.text(
        8,
        6.15,
        'Clinical Logic + Guideline Mapping + Constraint Checking',
        ha='center',
        va='center',
        fontsize=10,
    )
    ax.text(
        8,
        5.75,
        'Safety Verification: Validate pathway against clinical guidelines',
        ha='center',
        va='center',
        fontsize=9,
        style='italic',
    ...
    ax.text(
        8,
        3.85,
        'Map State → Care Pathway → Clinical Action',
        ha='center',
        va='center',
        fontsize=10,
    )
    ...
    for pathway in pathways:
        ...
        path_box = FancyBboxPatch(
            (pathway['x'] - 0.9, pathway['y'] - 0.5),
            1.8,
            1.0,
            boxstyle="round,pad=0.1",
            edgecolor=pathway['color'],
            facecolor=pathway['color'],
            linewidth=2,
            alpha=0.3,
        )
    ...
    ax.text(
        8,
        0.5,
        'AUDIT TRAIL: Decision ID + Timestamp + State + Pathway + Explanation + Clinician Override (if any)',
        ha='center',
        va='center',
        fontsize=9,
        fontweight='bold',
    ...
    # Save figure
    plt.savefig(
        f'{output_dir}/fig9_orasr_routing_diagram.png', dpi=600, bbox_inches='tight'
    )
...
def create_titrate_decision_tree(output_dir='outputs/figures'):
    ...
    ax.text(
        8,
        11.5,
        'TiTrATE Decision Tree: Temporal & Symptomatic Classification',
        ha='center',
        va='top',
        fontsize=17,
        fontweight='bold',
    ...
    ax.text(
        8,
        10.35,
        'Dizziness/Vertigo Presentation',
        ha='center',
        va='center',
        fontsize=12,
        fontweight='bold',
    ...
    for node in duration_nodes:
        ...
        ax.text(
            ...
            fontsize=8,
            fontweight='bold',
            color='red' if node['tier'] == 'R2' else 'green',
        )
    ...
    for node in symptom_nodes:
        ...
        ax.add_patch(node_box)
        ax.text(
            node['x'], node['y'], node['label'], ha='center', va='center', fontsize=8
        )
    ...
    ax.text(
        8,
        2.0,
        'TiTrATE Classification Principles',
        ha='center',
        va='center',
        fontsize=13,
        fontweight='bold',
    ...
    legend_elements = [
        mpatches.Patch(
            facecolor='#CC3311',
            edgecolor='black',
            label='Critical Risk (R1/R2)',
            alpha=0.5,
        ),
        mpatches.Patch(
            facecolor='#ECA400',
            edgecolor='black',
            label='Moderate Risk (R3)',
            alpha=0.5,
        ),
        mpatches.Patch(
            facecolor='#029E73', edgecolor='black', label='Low Risk (R4/R5)', alpha=0.5
        ),
    ]
    ax.legend(
        handles=legend_elements,
    ...
    # Save figure
    plt.savefig(
        f'{output_dir}/fig10_titrate_decision_tree.png', dpi=600, bbox_inches='tight'
    )
